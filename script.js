...
//   setTimeout(type, 500);
// }
// Auto scroll + reset for each iframe
  document.querySelectorAll('.auto-scroll-frame').forEach(function(iframe) {
    iframe.addEventListener('load', function() {
      try {
        ...
        var current = 0;
        var direction = 1;
        var speed = 0.8; // pixels per tick — slow cinematic scroll

        setInterval(function() {
          current += speed * direction;

          // Jab bottom aaye — ruko 1.5s phir wapas upar
          ...
          // Jab top aaye — ruko 1s phir phir neeche
          if (current <= 0) {
            current = 0;
            direction = 0;
            setTimeout(function() {
        ...
        // Same-origin error — silently ignore
      }
    });
  });
